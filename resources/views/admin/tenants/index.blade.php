...
@section('content')
    <div class="ml-24">
        <a href="javascript:void(0)" id="createNewTenant" class=" bg-blue-500  absolute right-8 top-24 px-8 text-white shadow-5xl mb-10 p-2 uppercase font-bold">Add New Client</a>
       
        <table id="tenantsTable" class="mb-12 mt-4 w-full table table-hover dataTable table-custom table-striped m-b-0 c_list" style="width:100%">

            <thead>
                <tr class="text-left border-b-6 p-4">
                    <th class="border-separate border-b-2">Name</th>
                    <th class="border-b-2">Email</th>
                    <th class="border-b-2 w-1/12">Domain</th>
                    <th class="border-b-2 w-1/12">Action</th>
                </tr>
            </thead>
            <tbody>
            </tbody>
    ...
    <!-- Main modal -->
    <div id="tenantModal"  aria-hidden="true" class=" ajaxModal hidden overflow-y-auto overflow-x-hidden fixed right-0 left-0 top-4 z-50 justify-center items-center h-modal md:h-full md:inset-0">
        <div class="relative px-4 w-full  mx-auto max-w-md h-full md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form class="px-6 pb-4 space-y-6 lg:px-8 sm:pb-6 xl:pb-8" id="tenantForm" name="tenantForm">
                    @csrf
                    ...
                    <div>
                        <label for="domain" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Domain</label>
                        <input type="text" name="domain" id="domain" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"  required>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready( function () {
        ...
        $("#createNewTenant").click(function(){
            $("#student_id").val('');
            $("#tenantForm").trigger("reset");
            $('#modalHeading').html("Add Tenant");
        ...
        $("#saveBtn").click(function(e){
            e.preventDefault();
            let formData = $("#tenantForm").serialize();
             $.ajax({
                type: "POST",
